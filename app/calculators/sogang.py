...
class SogangScoreCalculator:
    ...
    # 탐구 변환표 (인문/자연/자유전공)
    # col0: 인문/지식융합, col1: 자연, col2: 자유전공
    CONV_TABLE = {
    ...
    def calculate_track_score(
        ...
        # 5. 한국사 가산점 (1~4등급: 10점, 5등급부터 -0.5씩)
        ...
        # 6. A형/B형 점수 계산
        # A형: 국어×1.1 + 수학×1.3 + 탐구×0.6 + 영어 + 한국사
        # B형: 국어×1.3 + 수학×1.1 + 탐구×0.6 + 영어 + 한국사
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("서강대 2026 환산 점수 테스트")
    print("="*60)
    
    results = calculate_sogang_score(test_data)
    for track, data in results.items():
        if data["계산_가능"]:
            print(f"{track}: {data['최종점수']:.1f}점 ({data['적용방식']})")
            print(f"  A형: {data['A형_점수']:.1f}, B형: {data['B형_점수']:.1f}")
