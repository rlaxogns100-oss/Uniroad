    ...
    *,
    ...
    for round_index in range(1, MAX_RETRIEVAL_ROUNDS + 1):
        ...
        if round_calls:
            ...
            round_results = await execute_function_calls(round_calls)
...
async def generate_school_record_report(
    *,
    ...
    deep_dive_start = time.perf_counter()
    deep_dive_text = await _generate_subject_deep_dive(
        message=message,
        school_record_context=school_record_context,
        mode=report_mode,
    )
    ...
    deep_dive_ms = int((time.perf_counter() - deep_dive_start) * 1000)
...
def generate_school_record_report_stream(
    *,
    ...
    try:
        ...
        deep_dive_text = loop.run_until_complete(
            _generate_subject_deep_dive(
                message=message,
                school_record_context=school_record_context,
                mode=report_mode,
            )
